...
扫描包是为了将路径下当作翻译器的**静态方法**和**枚举**获取，从而将他们注册转化成为翻译器。

包扫描不适用于动态方法注册成为翻译器，因为动态方法由对象调用。

但是，您不需要做其他额外的操作，`Easy-Translation`会**自动**将所有**含有翻译器方法的Bean**装载至`TranslationContext`中，从而将动态方法注册转化为翻译器。
